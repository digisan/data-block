...
func (opt *Option) file(key, value string, repeatIdx bool) {
	if opt.dir != "" {
		absdir, _ := fd.AbsPath(opt.dir, false)
...
func (opt *Option) Fac4SaveWithIdxKey(start int) func(value string) {
	idx := int64(start - 1)
...
func (opt *Option) FileSyncToMap() int {
	files, _, err := fd.WalkFileDir(opt.dir, true)
	if err != nil {
		return 0
	}
	return len(ts.FM(
